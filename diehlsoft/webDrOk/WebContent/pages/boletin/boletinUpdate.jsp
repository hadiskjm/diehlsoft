...
function save(){
	  var boletinForm = document.getElementById("boletinForm");
	  boletinForm.action="<%= request.getContextPath()%>/boletinUpdate.do?event=bolsaveUpdate";
	  boletinForm.submit();
	  }
...
<body>
<html:form action="boletinUpdate.do" method="post" styleId="boletinForm" enctype="multipart/form-data">
...
<!-- =================================================== -->
	...
	<label><bean:message key="boletinForm.boletinUpdate.txt.intbolId"/>:</label>
	<html:text property="intbolId" disabled="true"/>
	<span  class="generalErrors"><html:errors property="intbolId"/></span>
	...
	<script type="text/javascript">
	alert("<bean:message key="boletinForm.boletinUpdate.error.error" /> <bean:write name="boletinForm" property="strbolNombre"/>");
	</script>
    ...
    <BR>
            ...
            <bean:message key='boletinForm.boletinUpdate.btn.save'/></html:submit> 
		     &nbsp;
			<html:button property="" styleClass="button green medium" onclick="cancel()" >
			<bean:message key='boletinForm.boletinUpdate.btn.cancel'/></html:button>
    </div>
